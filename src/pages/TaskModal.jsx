...
// สังเกตว่าเราไม่ต้อง import PEOPLE แล้ว เพราะเราจะพิมพ์เอง
...
function TaskModal({ show, onHide, task, defaultStatus, onSave, onDelete, categories, onAddCategory }) {
  ...
  // สร้าง State สำหรับเก็บชื่อและ ID ของผู้รับผิดชอบงาน
  ...
  useEffect(() => {
    if (show) {
      if (task) {
        ...
        // ดึงชื่อและ ID มาแสดงเวลาแก้ไขงาน
  ...
  const handleSubmit = (e) => {
    ...
    const taskData = {
      ...
      assigneeName, // บันทึกชื่อที่พิมพ์ลงไป
      assigneeId,   // บันทึก ID ที่พิมพ์ลงไป
      status,
    };
    ...
    <Modal show={show} onHide={onHide} centered backdrop="static">
      ...
      <Form onSubmit={handleSubmit}>
        <Modal.Body>
          ...
          <Row className="mb-3">
            {/* ช่องสำหรับพิมพ์ ID และ Full Name แทน Dropdown เดิม */}
